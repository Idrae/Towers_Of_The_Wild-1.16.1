...
public class IceTowerPieces {
    ...
    public static class Piece extends AbstractTowerPiece {
        ...
        public Piece(TemplateManager templateManager, ResourceLocation structurePart, BlockPos absolutePos, Rotation rotation) {
            super(templateManager, structurePart, rotation, RegistryHandler.ICE_TOWER_PIECE, CENTER_TOP_OFFSETS);
        ...
        public Piece(TemplateManager p_i50566_1_, CompoundNBT p_i50566_2_) {
            super(p_i50566_1_, RegistryHandler.ICE_TOWER_PIECE, p_i50566_2_, CENTER_TOP_OFFSETS);
        }
            ...
            if (this.structurePart.equals(ICE_TOWER_BOTTOM)) {
                ...
                for (int i=0; i<9; ++i) {
                    for (int j=0; j<9; ++j) {
                        ...
                        if (p_230383_1_.getBlockState(snowPos).getBlock() == Blocks.AIR) {
                            p_230383_1_.setBlockState(snowPos, Blocks.SNOW.getDefaultState(), 3);
                        }
                ...
                for (int i=1; i<8; ++i) {
                    for (int j=1; j<8; ++j) {
                        height = p_230383_1_.getHeight(Heightmap.Type.WORLD_SURFACE_WG, blockpos1.getX() + 1 + i, blockpos1.getZ() + 1 + j);
                        if (height < minHeight) {
                            minHeight = height;
                        }
                    }
